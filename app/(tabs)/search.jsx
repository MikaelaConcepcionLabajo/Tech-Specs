    ...
    return (
        <View style={styles.card}>
            ...
            <TouchableOpacity
                ...
                disabled={isWishlisted} // Disable button if already added
            >
...
export default function Home() {
    ...
    // Combine phones
    const combinedPhones = [...bestPhones, ...budgetPhones];

    // Filter phones based on search query
    const filteredPhones = combinedPhones.filter(phone =>
        phone.name.toLowerCase().includes(searchQuery.toLowerCase())
    );

    // Create two rows
    const rows = [];
    for (let i = 0; i < filteredPhones.length; i += 2) {
        rows.push(filteredPhones.slice(i, i + 2));
    }

    return (
        <ScrollView style={styles.container}>
            <Text style={styles.header}>Search for a Phone</Text>
            <TextInput
            ...
            />
            {rows.length > 0 ? (
                rows.map((phones, index) => (
                    <Row key={index} phones={phones} />
                ))
            ) : (
...
const styles = StyleSheet.create({
    ...
    card: {
        ...
        borderRadius: 15,
        padding: 16,
        marginRight: 8, // Space between cards in a row
        shadowColor: '#000',
        shadowOffset: {
            width: 0,
            height: 3,
